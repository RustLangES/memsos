...
impl BumpAllocator {
    ...
    pub fn init(&mut self, st: *mut SystemTable) {
        unsafe {
            ...
            let status = ((*(*st).boot_services).allocate_pool)(efi::LOADER_DATA, SIZE, ptr as *mut *mut c_void);
            if status.is_error() {
                panic!("Cannot init memory allocator");
            }
    ...
    pub fn free(&self, st: *mut SystemTable) {
        unsafe {
            ((*(*st).boot_services).free_pages)(self.block as u64, SIZE);
        }
    }
}
