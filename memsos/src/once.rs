...
impl<T> Once<T> {
    ...
    where
        F: FnOnce() -> T,
    {
        assert!(!self.is_set.load(Ordering::Acquire), "Struct Once can only be set once.");
...
impl<T> Deref for Once<T> {
    ...
    fn deref(&self) -> &Self::Target {
        assert!(!self.is_set.load(Ordering::Acquire), "The value cannot be accessed if it has not yet been initialized");
