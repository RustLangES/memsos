...
impl AcpiTables {
    ...
    pub fn get_tables(&self) -> impl Iterator<Item = usize> {
        ...
        let mut table_entries_ptr = (self.sdt_addr + size_of::<SdtHeader>() as u64) as *mut u8;
...
pub unsafe fn sdt_checksum(rsdt_addr: *mut u8, rsdt_len: usize) -> bool {
    let mut sum: u8 = 0;
...
impl RsdpHeader {
    ...
    pub fn get_sdt_header(&self) -> (*mut SdtHeader, u64) {
        match self.get_version() {
            AcpiVersion::V1 => self.get_rsdt_address(),
            AcpiVersion::V2 => self.get_xsdt_address(),
        }
    }
